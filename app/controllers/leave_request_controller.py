...
class LeaveRequestController(BaseController):
    ...
    def update_leave_request_status(self):
        ...
        if not user.manager == manager_id or not manager.role == "Super admin":
            return self.handle_response('Only a manager assigned to this user can approve or reject a leave application', status_code=400)
